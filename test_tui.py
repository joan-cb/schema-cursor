...
class TestSchemaBuilderTUI:
    ...
    def test_app_compose(self, sample_json_file):
        ...
        # We can't easily test the exact structure without running the app,
        # but we can verify the compose method doesn't raise errors
...
class TestTUIVisualRegression:
    ...
    def test_widget_hierarchy(self, sample_json_file):
        ...
        # Test compose method creates expected structure
        widgets = list(app.compose())
        
        # Should have Header, main container, and Footer
        assert len(widgets) >= 2  # At least Header and main content
